...
namespace Game.Networking.Adapters
{
    public partial class PlayerNetworkDriverFishNet
    {
        ...
        void HandlePackedShard(byte[] shard)
        {
            ...
            if (!all)
            {
                ...
                if (missingCount == 1 && parityCount > 0)
                {
                    ...
                    list[missingDataIdx] = new ShardInfo { total = (ushort)totalShards, index = (ushort)missingDataIdx, dataLen = recoveredDataLen, data = recData };
            ...
            {
                Debug.LogWarning($"[Driver.Warning] Failed to verify shard hash id={messageId}: {ex.Message}");
            }
            ...
            if (isCanary)
            {
                if (verboseNetLog) Debug.Log($"[Driver.Canary] ignoring canary shards id={messageId}");
                return;
            }
        ...
        void RequestFullSnapshotFromServer(bool preferNoFec = false)
        {
            ...
            {
                if (verboseNetLog)
                    Debug.Log($"[Driver.Debug] Full snapshot request suppressed due to cooldown ({now - _lastFullRequestTime:0.###}s)");
                return;
            }
