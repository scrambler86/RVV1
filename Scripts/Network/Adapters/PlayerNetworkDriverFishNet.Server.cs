...
public partial class PlayerNetworkDriverFishNet
{
    ...
    void CmdSendInput(Vector3 dir, Vector3 clientPredictedPos, bool running, uint seq, bool isCTM, Vector3[] pathCorners, double timestamp)
    {
        ...
        _telemetry?.Observe($"client.{OwnerClientId}.predicted_vs_integrated_cm", Vector3.Distance(serverIntegratedPos, clientPredictedPos) * 100.0);
        ...
        if (validateNavMesh && NavMesh.SamplePosition(finalPos, out var nh, navMeshMaxSampleDist, NavMesh.AllAreas))
